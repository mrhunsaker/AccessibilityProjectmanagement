...
def _metadata_dialog(print_job_id: int, on_done) -> None:
    open_metadata_dialog("print", print_job_id, on_done)
        ...
        with ui.row().classes("justify-end gap-3 mt-2"):
            ...
            def _save() -> None:
                if not pnames:
                    ...
                    return
                request_date = rdate.value.strip()
        ...
        with content_area:
            with ui.row().classes("items-center gap-3 mb-1"):
                ...
                ui.button(
                    ...
                    on_click=lambda: export_job_summary(
                        job_type="print",
                        job=j,
                        step_order=_PRINT_STEPS,
                        step_labels=_PRINT_STEP_LABELS,
                    ),
            ...
            with ui.card().classes("mt-4 p-5 rounded-xl border border-slate-200"):
                render_event_log(
                    ...
                    step_labels=_PRINT_STEP_LABELS,
                    on_done=_refresh,
                    step_badge_classes="text-xs bg-amber-50 text-amber-700 rounded px-1",
                )
        ...
        def _render_bulk_actions(current_jobs: list[dict]) -> None:
            ...
            with bulk_row:
                ui.badge(f"{len(selected)} selected").classes(
                    "bg-amber-100 text-amber-700 rounded px-2"
                )
                            ...
                            writer.writerow(
                                {
                                    ...
                                    "filament_used_g": row.get("filament_used_g", ""),
                                    "printed_at": row.get("printed_at", ""),
                                    "delivery_date": row.get("delivery_date", ""),
                                }
                            )
                    ui.download(tmp.name, filename="print_jobs_selected.csv")
