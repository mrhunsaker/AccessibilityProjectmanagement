"""
Shared UI helpers — progress bars, badges, confirmation dialogs.
"""
...
def progress_bar(done: int, total: int) -> None:
    """Render a compact progress bar with fraction label."""
...
def status_chip(label: str, done: bool) -> None:
    if done:
        ui.badge("✓ " + label).classes("bg-green-100 text-green-800 text-xs rounded px-2 py-0.5")
    else:
...
def confirm_dialog(
    ...
    """Show a simple inline confirm dialog."""
    with ui.dialog() as dialog, ui.card().classes("p-6 gap-4 min-w-80"):
        ...
        with ui.row().classes("gap-3 justify-end w-full mt-2"):
            ui.button("Cancel", on_click=dialog.close).props("flat").classes("text-slate-600")
            def _do():
                dialog.close()
                on_confirm()
    ...
    with ui.row().classes(f"gap-6 flex-wrap {cls}"):
        for key, val in labels:
            with ui.column().classes("gap-0"):
                ui.label(key).classes("text-xs text-slate-400 uppercase tracking-wider")
                ui.label(str(val) if val is not None else "—").classes("text-sm text-slate-700 font-medium")
