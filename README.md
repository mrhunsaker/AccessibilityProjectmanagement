...
- User and role management (administrator, operator, reviewer roles)
...
Documentation is automatically deployed to GitHub Pages via the workflow in `.github/workflows/docs.yml`. To trigger a manual build, push changes to the `main` branch.
...
| `ACCESSMAN_API_AUTH_REQUIRED` | No | `1` | Enable API authentication (`0` = disabled) |
| `ACCESSMAN_API_KEY` | No | - | API key for authentication |
| `ACCESSMAN_DB_PATH` | No | User data directory | Path to SQLite database |
| `ACCESSMAN_UNPROTECTED` | No | `0` | **Danger:** Disable authentication for development (`1` = enabled) |
...
- **Session Encryption**: NiceGUI STORAGE_SECRET
- **RBAC**: Three roles — administrator, operator, reviewer
...
**Core Dependencies:**
- `nicegui>=3.12.0` – Web UI framework
- `textual>=0.80.0` – Terminal UI components
...
#### Build Notes
- All builds include Python dependencies, application resources, and database schemas
- **Browser-Only**: All builds enforce `NICEGUI_BROWSER_ONLY=1`
- **Testing**: Always test built binaries on a clean machine without Python installed

### Running with Podman

APM can be deployed using **Podman** (rootless container runtime).
